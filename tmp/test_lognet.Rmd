...
```{python}
G2 = BinomialGLM(response_col='response',
                 offset_col='offset',
                weight_col='weight'
                )
G2.fit(X, Df)
...
```{python}
G4 = BinomialGLM(response_col='binary')
G4.fit(X, Df)
```
...
```{python}
G5 = BinomialGLM(response_col='binary', weight_col='weight')
G5.fit(X, Df)
```
...
```{python}
G6 = BinomialGLM(response_col='binary', offset_col='offset')
G6.fit(X, Df)
```
...
```{python}
GN2 = LogNet(response_col='response',
            offset_col='offset',
             weight_col='weight',
             ).fit(X, Df)
...
```{python}
GN3 = LogNet(response_col='response',
               offset_col='offset',
               ).fit(X, Df)
...
```{python}
GN4 = LogNet(response_col='response',
             offset_col='offset',
	         weight_col='weight',
               ).fit(X, Df)
