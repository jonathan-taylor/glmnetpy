...
```{r magic_args="-i event,status,start,X,W -o C"}
library(survival)
...
```{r magic_args="-i foldid,X,event,start,status,W -o CVM,CVSD"}
...
```{python}
assert np.allclose(GN3.cv_scores_['Cox Deviance'].iloc[:10], CVM[:10]/2, rtol=1e-3, atol=1e-3)
assert np.allclose(GN3.cv_scores_['SD(Cox Deviance)'].iloc[:10], CVSD[:10]/2, rtol=1e-3, atol=1e-3) 
```

```{python}
fig, ax = plt.subplots()
ax.scatter(GN3.cv_scores_['Cox Deviance'].iloc[:10], CVM[:10])
ax.axline((GN3.cv_scores_['Cox Deviance'].iloc[:10].min(),CVM[:10].min()),slope=0.5, ls='--', c='k')
...
ax.axline((GN3.cv_scores_['SD(Cox Deviance)'].iloc[:10].min(),CVSD[:10].min()),slope=0.5, ls='--', c='k')
...
```{python}
assert np.allclose(GN4.cv_scores_['Cox Deviance'].iloc[:10], CVM[:10]/2, rtol=1e-3, atol=1e-3)
assert np.allclose(GN4.cv_scores_['SD(Cox Deviance)'].iloc[:10], CVSD[:10]/2, rtol=1e-3, atol=1e-3) 
```

```{python}
fig, ax = plt.subplots()
ax.scatter(GN4.cv_scores_['Cox Deviance'].iloc[:10], CVM[:10])
ax.axline((GN4.cv_scores_['Cox Deviance'].iloc[:10].min(),CVM[:10].min()),slope=0.5, ls='--', c='k')
