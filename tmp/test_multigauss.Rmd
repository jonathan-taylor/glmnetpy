...
```{python}
assert np.allclose(GN3.cv_scores_['Mean Squared Error']*q, CVM)
assert np.allclose(GN3.cv_scores_['SD(Mean Squared Error)']*q, CVSD) 
```
...
```{python}
assert np.allclose(GN4.cv_scores_['Mean Squared Error']*q, CVM)
assert np.allclose(GN4.cv_scores_['SD(Mean Squared Error)']*q, CVSD) 
```
