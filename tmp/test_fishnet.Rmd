...
```{python}
G5 = GLM(response_col='response', weight_col='weight', family=sm.families.Poisson())
G5.fit(X, Df)
```
...
```{python}
G6 = GLM(response_col='response', offset_col='offset', family=sm.families.Poisson())
G6.fit(X, Df)
```
...
```{python}
GN2 = FishNet(response_col='response',
              offset_col='offset',
              weight_col='weight',
             ).fit(X, Df)
...
```{python}
GN3 = FishNet(response_col='response',
               offset_col='offset',
               ).fit(X, Df)
...
```{python}
GN4 = FishNet(response_col='response',
             offset_col='offset',
	         weight_col='weight',
               ).fit(X, Df)
