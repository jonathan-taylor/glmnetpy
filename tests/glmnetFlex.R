...
elnet.fit <- function(x, y, weights, lambda, alpha = 1.0, intercept = TRUE,
    ...
    if (inherits(x, "sparseMatrix")) {
        ...
        args <- list(alm0=alm0,almc=almc,alpha=alpha,m=m,no=nobs,ni=nvars,
                     x=x,xm=as.matrix(xm),xs=as.matrix(xs),r=as.matrix(r),xv=as.matrix(xv),v=as.matrix(v),intr=intr,ju=as.matrix(ju),vp=as.matrix(vp),cl=as.matrix(cl),nx=nx,thr=thr,
                     maxit=maxit,a=as.matrix(a.new),aint=aint,g=as.matrix(g),ia=as.matrix(ia),iy=as.matrix(iy),iz=iz,mm=as.matrix(mm),
                     nino=nino,rsqc=rsqc,nlp=nlp,jerr=jerr)
        c.args <- deep_copy(args)
      ...
      args <- list(alm0=alm0,almc=almc,alpha=alpha,m=m,no=nobs,ni=nvars,
                   x=x,r=as.matrix(r),xv=as.matrix(xv),v=as.matrix(v),intr=intr,ju=as.matrix(ju),vp=as.matrix(vp),cl=as.matrix(cl),nx=nx,thr=thr,
                   maxit=maxit,a=as.matrix(a.new),aint=aint,g=as.matrix(g),ia=as.matrix(ia),iy=as.matrix(iy),iz=iz,mm=as.matrix(mm),
                   nino=nino,rsqc=rsqc,nlp=nlp,jerr=jerr)
      c.args <- deep_copy(args)
...
#'
#' @param x Input matrix, of dgimension \code{nobs x nvars}; each row is an
#' observation vector. If it is a sparse matrix, it is assumed to be unstandardized.
#' It should have attributes \code{xm} and \code{xs}, where \code{xm(j)} and
#' \code{xs(j)} are the centering and scaling factors for variable j respsectively.
